package Disilon;

import java.util.Vector;

import static Disilon.Main.df2;
import static Disilon.Main.game_version;

public class Potion {
    Player owner;
    String type;
    byte type_id;
    int tier;
    double cooldown;
    int threshold;
    int used;
    int count;

    public Potion(Player player, String type, int threshold) {
        this.owner = player;
        this.type = type.toLowerCase().substring(0, 2);
        switch (this.type) {
            case "hp" -> type_id = 1;
            case "mp" -> type_id = 2;
        }
        this.tier = Integer.parseInt(type.substring(4, 5));
        if (type.length() > 5) {
            this.tier += 10;
        }
        this.threshold = threshold;
    }

    public void checkPotion(Actor player, double time) {
        if (cooldown > 0) {
            cooldown -= time;
        }
        if (cooldown <= 0) {
            switch (type_id) {
                case 1 -> {
                    if (player.hp < player.getHp_max() * threshold / 100) {
                        player.setHp(player.hp + hp_gain() * player.getPotion_effect(true));
                        cooldown = hp_cd();
                        used++;
                        count--;
                    }
                }
                case 2 -> {
                    if (player.mp < player.getMp_max_no_buffs() * threshold / 100) {
                        player.setMp(player.mp + mp_gain() * player.getPotion_effect(false));
                        cooldown = mp_cd();
                        used++;
                        count--;
                    }
                }
            }
        }
    }

    public void tickPotion(double time) {
        if (cooldown > 0) {
            cooldown -= time;
        }
    }

    private int hp_gain() {
        return switch (tier) {
            case 1 -> 200;
            case 2 -> 500;
            case 3 -> 1200;
            case 4 -> 3000;
            case 5 -> 7500;
            case 11 -> 250;
            case 12 -> 600;
            case 13 -> 1350;
            case 14 -> 3500;
            case 15 -> 9000;
            default -> 0;
        };
    }

    private int mp_gain() {
        return switch (tier) {
            case 1 -> 125;
            case 2 -> 300;
            case 3 -> 700;
            case 4 -> game_version >= 1694 ? 2250 : 1500;
            case 5 -> game_version >= 1694 ? 5000 : 3000;
            case 11 -> 150;
            case 12 -> 350;
            case 13 -> 800;
            case 14 -> game_version >= 1694 ? 2500 : 1750;
            case 15 -> game_version >= 1694 ? 6000 : 3500;
            default -> 0;
        };
    }

    private double hp_cd() {
        return switch (tier) {
            case 1 -> 15;
            case 2 -> 20;
            case 3 -> 30;
            case 4 -> 40;
            case 5 -> 50;
            case 11 -> 7.5;
            case 12 -> 10;
            case 13 -> 15;
            case 14 -> 20;
            case 15 -> 25;
            default -> 0;
        };
    }

    private double mp_cd() {
        return switch (tier) {
            case 1 -> 15;
            case 2 -> game_version >= 1694 ? 20 : 25;
            case 3 -> game_version >= 1694 ? 30 : 45;
            case 4 -> game_version >= 1694 ? 40 : 60;
            case 5 -> game_version >= 1694 ? 50 : 90;
            case 11 -> 7.5;
            case 12 -> game_version >= 1694 ? 10 : 12.5;
            case 13 -> game_version >= 1694 ? 15 : 22.5;
            case 14 -> game_version >= 1694 ? 20 : 30;
            case 15 -> game_version >= 1694 ? 25 : 45;
            default -> 0;
        };
    }

    public double base_craft_time() {
        return switch (tier) {
            case 1 -> 2;
            case 2 -> 4;
            case 3 -> 6;
            case 4 -> 8;
            case 5 -> 10;
            case 11 -> 2;
            case 12 -> 4;
            case 13 -> 6;
            case 14 -> 8;
            case 15 -> 10;
            default -> 0;
        };
    }

    public double base_alch_time() {
        return switch (tier) {
            case 1 -> 3;
            case 2 -> 6;
            case 3 -> 9;
            case 4 -> 12;
            case 5 -> 15;
            case 11 -> 4.5;
            case 12 -> 9;
            case 13 -> 13.5;
            case 14 -> 18;
            case 15 -> 22.5;
            default -> 0;
        };
    }

    public static Vector<String> getAvailablePotions() {
        Vector<String> result = new Vector<>();
        result.add("None");
        for (int i = 1; i < 6; i++) {
            result.add("HP T" + i);
            result.add("HP T" + i + " I");
        }
        for (int i = 1; i < 6; i++) {
            result.add("MP T" + i);
            result.add("MP T" + i + " I");
        }
        return result;
    }

    public String getRecordedData(double time) {
        return type.toUpperCase() + " potion tier: " + getTier() + ", used: " + used + ", per hour: " + df2.format(used / time * 3600) +
                "\n";
    }

    public String getTier() {
        return tier > 9 ? (tier - 10) + " I" : String.valueOf(tier);
    }
}
